var RoleController = function () {
    ...
    function registerEvents() {
        ...
        $("#ddl-show-page").on('change', function () {
            khoaluan.configs.pageSize = $(this).val();
            khoaluan.configs.pageIndex = 1;
            loadData(true);
        });
        ...
        $('#btnSave').on('click', function (e) {
            if ($('#frmMaintainance').valid()) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        khoaluan.notify('Update role successful', 'success');
                    ...
                    error: function () {
                        khoaluan.notify('Has an error', 'error');
                        khoaluan.stopLoading();
                    }
                });
                return false;
        ...
        $('body').on('click', '.btn-delete', function (e) {
            ...
            khoaluan.confirm('Are you sure to delete?', function () {
                $.ajax({
                    ...
                    success: function (response) {
                        khoaluan.notify('Delete successful', 'success');
                        khoaluan.stopLoading();
                        loadData();
                    },
                    error: function (status) {
                        khoaluan.notify('Has an error in deleting progress', 'error');
                        khoaluan.stopLoading();
                    }
                });
            });
        ...
        $("#btnSavePermission").off('click').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    khoaluan.notify('Save permission successful', 'success');
                    $('#modal-grantpermission').modal('hide');
                    khoaluan.stopLoading();
                },
                error: function () {
                    khoaluan.notify('Has an error in save permission progress', 'error');
                    khoaluan.stopLoading();
                }
            });
        });
